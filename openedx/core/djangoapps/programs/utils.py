...
class ProgramProgressMeter(object):
    ...
    def __init__(self, user, enrollments):
        self.user = user
        self.enrollments = sorted(enrollments, key=lambda e: e.created, reverse=True)

        # enrollment.course_id is really a course key ಠ_ಠ
        self.course_ids = [unicode(e.course_id) for e in self.enrollments]
        self.engaged_programs = None
    ...
    def programs(self):
        ...
        self.engaged_programs = []
        for course_id in self.course_ids:
            for program in flattened.get(course_id, []):
                if program not in self.engaged_programs:
                    self.engaged_programs.append(program)

        return self.engaged_programs
    ...
    def progress(self):
        ...
        """
        if self.engaged_programs is None:
            self.programs  # pylint: disable=pointless-statement
